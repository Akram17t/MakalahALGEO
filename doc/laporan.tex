...
Nilai eigen dapat dicari dengan menyelesaikan:
\[
\det(A - \lambda I) = 0.
\]
...
\]
Spectral radius memberikan informasi tentang seberapa "kuat" koneksi dalam jaringan dan seberapa cepat informasi atau pengaruh dapat menyebar. Nilai yang tinggi mengindikasikan adanya hub nodes (simpul pusat) yang sangat dominan dalam jaringan.

Dalam konteks jaringan drainase, spectral radius dapat kasih info tentang:
\begin{itemize}
...
Tahap terakhir bertujuan untuk menerjemahkan hasil analisis spektral dan hidraulik jadi metrik kerentanan yang dapat diinterpretasikan dan diterapkan dalam perencanaan pemeliharaan.
...
Dataset yang digunakan dalam penelitian ini adalah \textit{data dummy sintetis} yang dirancang untuk merepresentasikan karakteristik realistis jaringan drainase perkotaan berdasarkan literatur dan standar teknis. Dataset terdiri dari 200 simpul dan 851 koneksi. Data mencakup koordinat geografis serta 6 parameter hidraulik utama: elevasi (5.0--23.6 m), kapasitas aliran (0.5--9.7 m³/s), intensitas hujan (30.8--109.0 mm/h), risiko sedimentasi (0.223--0.894), lebar saluran, dan beban hidraulik.

Dataset lengkap dapat diakses di:
\begin{verbatim}
...
Nilai $\lambda_2 = 0.075$ jauh di bawah threshold 0.1, menandakan jaringan sangat rentan terhadap pemisahan. Jika satu simpul kritis rusak, jaringan dapat terpecah jadi komponen terpisah.
...
\section{KESIMPULAN}
Penelitian ini berhasil menerapkan eigenvalue decomposition dan power iteration method untuk mengidentifikasi titik kritis jaringan drainase perkotaan. Integrasi analisis topologi spektral dengan parameter hidraulik menghasilkan metode penilaian kerentanan yang lebih komprehensif dibanding pendekatan konvensional.

Hasil analisis menunjukkan algebraic connectivity $\lambda_2 = 0.075$ mengindikasikan kerentanan tinggi terhadap fragmentasi, di mana kegagalan simpul kritis dapat memecah jaringan menjadi komponen terpisah. Korelasi kuat antara eigenvalue centrality dan vulnerability ($r=0.715$) membuktikan metode spektral mampu menangkap informasi struktural penting. Distribusi kerentanan menunjukkan 30\% simpul masuk kategori tinggi yang memerlukan prioritas pemeliharaan. Model terbukti robust terhadap variasi bobot moderat dan menghasilkan ranking kuantitatif untuk perencanaan pemeliharaan preventif.

Keterbatasan penelitian meliputi model statis yang tidak mempertimbangkan dinamika temporal seperti fluktuasi beban hidraulik atau degradasi infrastruktur. Pengembangan model dinamis dengan skenario temporal (mis. skenario curah hujan berkala) akan memberikan insight yang lebih aplikatif. Validasi dengan data riil sistem drainase aktual sangat diperlukan untuk memverifikasi akurasi prediksi. Pendekatan ini dapat menjadi fondasi pengembangan sistem monitoring dan early warning berbasis waktu nyata.
